...
def search_lazada_candidates(app_key, app_secret, user_token, keywords, min_commission_rate=20.0):
    """
    Mencari produk di Lazada Feed mengikut pelbagai offerType (1, 2, 3) 
    dan menapis produk mengikut kata kunci serta kadar komisen.
    """
    ...
    # Semak pelbagai offerType dan halaman untuk kumpul sampel produk yang besar
        ...
        for page in ["1", "2"]:
            ...
            try:
                ...
                for prod in products:
                    ...
                    # Padanan Kata Kunci
                    kw_match = any(any(w.lower() in p_name.lower() for w in kw.split()) for kw in keywords)
                ...
                print(f"⚠️ [SEARCH WARN] Ralat carian offerType={offer_type}, page={page}: {e}")
                continue

    print(f"📦 Jumpa {len(candidates)} calon produk berpotensi dari carian keyword & feed.")
    return candidates