...
def search_lazada_via_duckduckgo(keyword):
    """
    Menggunakan library duckduckgo_search / ddgs untuk mencari produk Lazada
    ...
    try:
        with DDGS() as ddgs:
            # Carian Imej + Link Produk Lazada di DuckDuckGo
...
def search_lazada_candidates_by_keywords(keywords):
    """
    Menjalankan carian DuckDuckGo untuk kata kunci dengan jeda masa anti-ratelimit & early break.
    """
    all_candidates = []
    seen_ids = set()
    ...
    for idx, kw in enumerate(keywords):
        if idx > 0:
            # Jeda masa 4 saat antara carian untuk elak RateLimit 403
            time.sleep(4)
        ...
        # Sebaik sahaja jumpa sekurang-kurangnya 5 calon produk, hentikan carian berikutnya untuk elak rate limit
        if len(all_candidates) >= 5:
            print(f"✅ Berjaya menjumpai {len(all_candidates)} calon produk. Menghentikan carian kata kunci seterusnya bagi mengelakkan sekatan RateLimit.")
            break
