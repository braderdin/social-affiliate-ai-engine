...
# Koleksi gambar realistik mengikut kategori tema
THEMED_IMAGE_POOLS = {
    "GARDEN": [
...
def get_lifestyle_image_url(category="ALL"):
    """Memulangkan URL gambar realistik yang rawak mengikut kategori tema penceritaan."""
    category_key = str(category).upper()
    
    if category_key in THEMED_IMAGE_POOLS:
        pool = THEMED_IMAGE_POOLS[category_key]
    else:
        # Gabungkan semua jika kategori umum
